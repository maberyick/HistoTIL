...
def main(args):
    print(os.path.join(args.mask_path, '*.png'))
    files = sorted(glob.glob(os.path.join(args.mask_path, '*.png')))
    ...
    #files = sorted(glob.glob(os.path.join(args.input_path, '*'+args.ext)))
    #print(f'{len(files)} files have been read.')
    #if args.end_ind == -1:
    #    args.end_ind = len(files)-1
    #with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_worker) as executor:
    #    {executor.submit(startPatchExtractionWithoutMask, files, ind): ind for ind in range(args.start_ind, args.end_ind+1)}
...
def startPatchExtraction(ind, files, args):
    ...
    print('Image number: %s, Out of: %s, Saving path: %s, Image ID: %s, From the path: %s' % (ind+1, len(files), args.output_path, baseFname, args.input_path))
...
parser.add_argument('--file_list', type=str, default='./testimage/file_list.tsv', help='path to the TSV file containing the list of files')
