function [features] = getContextTILFeatures(nucleiCentroids, nucleiArea, nucleiEcc, nucleiDiam, MeanI, EntropyR, RatioMRB, RatioMRG)
dL = 20; % Mean Lymphocyte Diameter based on TMA at 20X
Kparameter = [dL*10,dL*20,dL*30]; % Radii to be multiplied with dL
...
if size(nucleiCentroids,1)>1
    ...
    for k=1:length(Kparameter)
        parfor j=1:length(nucleiCentroids)
