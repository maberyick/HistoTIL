function [feats,feature_list] = ccg_features(bounds, edges)
% MATLAB Implementation of Sahir's CCG features
% updated - faster implementation of connected components
% George Lee (2013)
% Edited: 03/09/13 - Sahir Ali

%%%  Notes by Sahir %%%%%%%%%%%%%%%%%%%%%%%%%%
%% There is a PDF that summarizes these features algorithimcally. Reach out to me for that.
%% Clustering Coeff has been found to be a very good feature across diseases (Prostate, Oral).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Sahir's advice on how to use construct_ccgs:
%{
Here is an example of how you might want to call these functions:

% %%%%   Inputs to the function  %%%%%%
%nodes - cell object which has the coordinates of the nuclear center
% r and alpha - you will have to set them by trial and error

img = img(300:500,300:500,1:3);
binmap = img(:,:,1) == 0 & img(:,:,2) == 255 & img(:,:,3) == 0; % selecting only pixels that are green (0, 255, 0)
STATS = regionprops(binmap,'Centroid'); % get the centroid of each blob in thebinmap (note: this is not strictly needed, only a safeguard against nuclei that have been marked with a blob (i.e. more than 1 pixel))
binmap_nuclei = false(size(img(:,:,1)));
for j=1:length(STATS)
    binmap_nuclei(round(STATS(j).Centroid(2)),round(STATS(j).Centroid(1))) = true;
end

[x y] = find(binmap_nuclei);
nodes(1).centroid_r = x';
nodes(1).centroid_c = y';

[VY,VX,x,y,s_edges,params] = construct_ccgs(nodes, 0.2, 0.46);

%%%% visualize the graph %%%%%%%%%
plot(y,x, 'go', 'MarkerSize', 5, 'MarkerFaceColor', 'b'); hold on; axis off;
for x = 1: numel(nodes)
    plot(nodes(x).centroid_r, nodes(x).centroid_c,'bo', 'MarkerSize',5, 'MarkerFaceColor', 'b');
end
plot(VX', VY', 'r-', 'LineWidth', 1.5);
hold off;

%%%%CCG feature Mining %%%%%%%%%%%
[feats,feature_list] = ccg_features(nodes, edges)

%}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%

disp('begin to calculate CCG features');
tic
%1) Number of Nodes
N = length(edges);
feats(1) = N;
